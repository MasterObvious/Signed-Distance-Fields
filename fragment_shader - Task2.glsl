...
float planelessScene(vec3 pt){
	float unionCube = sphere_cube(pt, vec3(-3.0, 0.0, -3.0), 1);
	float differenceCube = sphere_cube(pt, vec3(3.0, 0.0, 3.0), 2);
	float blendCube = sphere_cube(pt, vec3(-3.0, 0.0, 3.0), 3);
	float interCube = sphere_cube(pt, vec3(3.0, 0.0, -3.0), 4);

	return min(min(unionCube, differenceCube), min(blendCube, interCube));
}
...
vec3 getColor(vec3 pt) {
	if (abs(plane(pt, vec3(0.0, -1.0, 0.0))) < CLOSE_ENOUGH){
		...
		return mix(vec3(0.4, 1, 0.4), vec3(0.4, 0.4, 1), d);
	}else{
		  return vec3(1);
	}
